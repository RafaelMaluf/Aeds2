...
void quickSort(jogador *array, int leftIndex, int rightIndex)
{
    int i = leftIndex;
    int j = rightIndex;
    jogador pivo = array[(leftIndex + rightIndex) / 2];

    while (i <= j)
    {
        while (strcmp(getestadoNascimento(&array[i]), getestadoNascimento(&pivo)) < 0 || strcmp(getestadoNascimento(&array[i]), getestadoNascimento(&pivo)) == 0 && strcmp(getnome(&array[i]), getnome(&pivo)) < 0)
        {
            i++;
        }
        while (strcmp(getestadoNascimento(&array[j]), getestadoNascimento(&pivo)) > 0 || strcmp(getestadoNascimento(&array[j]), getestadoNascimento(&pivo)) == 0 && strcmp(getnome(&array[j]), getnome(&pivo)) > 0)
        {
            j--;
        }
        if (i <= j)
...
void sort(jogador *array, int n)
{
    ...
    for (i = 1; i < n; i++)
    {
        key = array[i];
        j = i - 1;
        while (j >= 0 && strcmp(getnome(&array[j]), getnome(&key)) > 0)
        {
            array[j + 1] = array[j];
            j = j - 1;
        }
