...
int main(){
    ...
    char array[MAX_STRINGS][MAX_LENGTH];
    ...
    for (int i = 0; i < (MAX_STRINGS-1) ; i ++ ) {
        for(int j = i + 1; j < MAX_STRINGS; j ++ ){
            ...
            if ( strcmp( array[i], array[j] ) > 0 ){
                ...
                strcpy ( copia, array[i] );
                strcpy( array[i], array[j] );
                strcpy( array[j], copia) ;
            }
        }
    }
    ...
    fgets(entraString, MAX_LENGTH, stdin); // primeira entrada = null

    fgets(entraString, MAX_LENGTH, stdin);
    while(!isFim(entraString)){
        bool resp = false;
        int y = 0;
     
        entraString[strcspn(entraString, "\n")] = '\0';
        
        int esq = -1, dir = MAX_STRINGS; 
        while (esq < dir-1) {  
      int meio = (esq + dir)/2;
      
      if (strcmp( array[meio], entraString ) == 0) 
        resp = true;
      if (strcmp( array[meio], entraString ) < 0) esq = meio;
      else dir = meio; 
    }
        if (resp ? printf("SIM\n") : printf("NAO\n"));
        fgets(entraString, MAX_LENGTH, stdin);
