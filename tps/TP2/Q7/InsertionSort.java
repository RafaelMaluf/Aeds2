...
/**
 * Ordenaçao por inserção em java
...
class InsertionSort extends Jogadores {
    ...
    public static Jogadores [] sort(Jogadores[] array, int n) {
        ...
        // decalracao de quantos elementos serao ordenados
        int k = 10;
        int comparacoes = 0;
        int movimentacoes = 0;
        int j = 0;
        for (int i = 0; i < (k - 1); i++) {
            ...
            for (j = (i + 1); j < n; j++) {
                if(array[menor].getAnoNascimento() == array[j].getAnoNascimento()){
                    comparacoes++;
